...
# ------------------------------------------------------------------
# 색상 팔레트 / 글꼴 (report_template/generate_report.js와 동일)
# ------------------------------------------------------------------
NAVY = RGBColor(0x1E, 0x2A, 0x44)
NAVY_DARK = RGBColor(0x14, 0x1D, 0x33)
NAVY_LIGHT = RGBColor(0x2E, 0x3D, 0x5C)
ICE = RGBColor(0xE8, 0xEC, 0xF2)
TERRACOTTA = RGBColor(0xC1, 0x65, 0x2F)
...
def _cover_slide(prs, data):
    ...
    for cx, cy, cw, ch, color in [
        (8.6, 3.0, 7.5, 7.5, NAVY_LIGHT), (9.6, 4.0, 5.5, 5.5, NAVY_DARK), (10.4, 4.8, 3.9, 3.9, NAVY),
    ]:
        ellipse = slide.shapes.add_shape(MSO_SHAPE.OVAL, Inches(cx), Inches(cy), Inches(cw), Inches(ch))
        ellipse.fill.solid()
        ellipse.fill.fore_color.rgb = color
        ellipse.line.color.rgb = TERRACOTTA if color == NAVY else color
        ellipse.line.width = Pt(1) if color == NAVY else Pt(0)
...
def _location_slide(prs, data):
    ...
    if loc.get("map_image"):
        pic = slide.shapes.add_picture(loc["map_image"], Inches(map_x), Inches(map_y), Inches(map_w), Inches(map_h))
        border = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(map_x), Inches(map_y), Inches(map_w), Inches(map_h))
        border.fill.background()
        border.line.color.rgb = GRID_LINE
        border.line.width = Pt(1)
        border.shadow.inherit = False
    else:
        _card(slide, map_x, map_y, map_w, map_h)
        _textbox(slide, map_x, map_y + map_h / 2 - 0.3, map_w, 0.6, "지도 이미지를 생성하지 못했습니다.",
                 size=13, color=MUTED, align=PP_ALIGN.CENTER, valign=MSO_ANCHOR.MIDDLE)
...
def _commercial_slide(prs, data):
    ...
    if com.get("top_industries"):
        ...
        _bar_chart(slide, 6.9, 1.7, 5.8, 4.6,
...
def _trade_area_map_slide(prs, data):
    ...
    if tam.get("map_image"):
        slide.shapes.add_picture(tam["map_image"], Inches(map_x), Inches(map_y), Inches(map_w), Inches(map_h))
        border = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(map_x), Inches(map_y), Inches(map_w), Inches(map_h))
        border.fill.background()
        border.line.color.rgb = GRID_LINE
        border.line.width = Pt(1)
    ...
    _textbox(slide, map_x, map_y + map_h + 0.05, map_w, 0.3,
             ...
             "정확한 폴리곤으로 표시할 수 없습니다.", size=8.5, italic=True, color=MUTED)
...
def _seoul_detail_slide(prs, data):
    ...
    if sd.get("weekday"):
        ...
        _bar_chart(slide, 6.6, 3.25, 6.1, 3.0, [d["label"] for d in sd["weekday"]], [d["value"] for d in sd["weekday"]],
